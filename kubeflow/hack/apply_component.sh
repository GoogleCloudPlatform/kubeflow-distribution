...
# Run this as KFCTXT=$(KFCTXT) ./apply_component -path <component path under kubeflow>
# For example: KFCTXT=cluster-context ./apply_component -path common/kubeflow-namespace
# To set build directory other than ./build, use the following command:
# BUILD_DIR=$(GENERATED_RESOURCE_LOCATION) KFCTXT=$(KFCTXT) ./apply_component -path <component path under kubeflow>
...
main() {
  ...
  if [ -f $makefile_path ] ; then
      ...
      if [ $HYDRATE_ONLY -ne 1 ]; then
        make -C $COMPONENT_PATH apply build_dir=$BUILD_DIR
      else
        make -C $COMPONENT_PATH hydrate build_dir=$BUILD_DIR
      fi
  else
